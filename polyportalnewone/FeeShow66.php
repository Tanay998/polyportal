...
<?php include('header.php') ?>
  ...
  <div class="container mt-5 rounded p-3 mb-5"> <!-------- container start ----------->
    <div class="row">
     <div class="col-md-12">
     <a href="FeeShow6.php" class="btn btn-primary" id="back3">BACK</a><br /><br />
     <a href="#" onclick="printpage()" style="text-decoration:none;" id="print3"><i class="fas fa-print" style="float:right;"></i></a>
      <hr />
      <?php 
        if(isset($_POST['submit']))
        {
          ...
          $query = "SELECT * FROM `feeinformation6` WHERE `StudentName`='$name' AND `FatherName`='$FatherName' AND `RegistrationNo`='$RegistrationNo'";
...
<div class="row" >
  <div class="col-md-12">
  <a href="update16.php?studname=<?php echo $result['StudentName']; ?>" style="float:right;color:white;font-weight:bold;" class="btn btn-danger" id="update3">UPDATE RECORD</a><br /><br />
    <table class="table  table-sm table-responsive" style="padding:20px;background-color:#F8F9EC;margin-left:0.5%;">
      <tbody class="table-bordered-success">
        <tr style="width:105%;" class="table-bordered border-top-0 bg-success">
          <td colspan="3" style="font-size:14px;font-weight:bold;">ThirdYear</td>
        </tr>
...
-------------------------------------------------------------------------------- OPTIONAL AND NEEDED JAVASCRIPT FILE ------------------------------------------------------------------------------------>    
    ...
    <script>
      function printpage()
      {
        var BackButton = document.getElementById("back3");
        var PrintOne = document.getElementById("print3");
        var UpdateOne = document.getElementById("update3");
