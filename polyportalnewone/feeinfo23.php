<?php 
    include('conn.php');
    extract($_POST);
    if(isset($_POST['readrecord']) && isset($_POST['course']) && isset($_POST['myInput'])){
        $data = '<table class="table table-bordered table-striped table-responsive">
            <tr>
                ...
                <th>Father Name</th>
                <th>Mother Name</th>
                <th>Parenst Mobile No.</th>
                <th>Date of Birth</th>
                <th>Email</th>
                <th>Gender</th>
            </tr>';
        if(($course=="Diploma in Electrical Engineering" && $myInput=="Sem1") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem1") || ($course=="Diploma in Electrical Engineering" && $myInput=="Sem2") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem2"))////////////////// Main If Start//////
        {    
        $displayquery = "SELECT * FROM `record1` WHERE `Course`='$course' AND `Semester`='$myInput'";
        ...
        if(mysqli_num_rows($result) > 0){ ///// If Start //////
            ...
            while($row = mysqli_fetch_array($result)){
                $data .='<tr>
                    ...
                    <td>'.$row['Year'].'</td>
                    <td>'.$row['Semester'].'</td>
                    <td>'.'<a class="btn btn-warning" target="_blank" href="showdata112.php?pname='.$row['First Name'].' && psem='.$row['Semester'].'">'.$row['First Name'].'</a>'.'</td>
    ...
    else if(($course=="Diploma in Electrical Engineering" && $myInput=="Sem3") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem3") || ($course=="Diploma in Electrical Engineering" && $myInput=="Sem4") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem4")){     //////////// Main Else Start ////////////////

        $displayquery = "SELECT * FROM `record2` WHERE `Course`='$course' AND `Semester`='$myInput'";
    ...
    else if(($course=="Diploma in Electrical Engineering" && $myInput=="Sem5") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem5") || ($course=="Diploma in Electrical Engineering" && $myInput=="Sem6") || ($course=="Diploma in Mechanical Engineering" && $myInput=="Sem6")){     //////////// Main Else Start ////////////////

        $displayquery = "SELECT * FROM `record3` WHERE `Course`='$course' AND `Year`='$myInput'";
        //$addquery ="SELECT SUM(GrandTotal) FROM `feeinformation1` WHERE `Branch`='$myInput' AND `Year`='$finy' AND `Semester`='$sem'";
        //$result1 = mysqli_query($db,$addquery);
        $result = mysqli_query($db, $displayquery);
        $rowcount1=mysqli_num_rows($result);
        if(mysqli_num_rows($result) > 0){ ///// If Start //////
            ...
            while($row = mysqli_fetch_array($result)){
                ...
                <script>
                    function NewTab2(){
                        window.open('showdata112.php?pname=".$row['First Name']." && psem=".$row['Semester']."','','width=900px,height=1000px');
                    }
                </script>
            ";
                $data .='<tr>
